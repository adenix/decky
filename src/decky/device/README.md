...
2. **Connection**: Opens HID interface to device
3. **Configuration**: Sets brightness, registers callbacks
4. **Communication**: Sends image data, receives button events
5. **Disconnection**: Closes HID interface, releases resources
...
2. **USB Errors**: Log and retry with backoff
3. **Render Errors**: Fall back to text-only display
4. **Disconnection**: Clean up and signal for reconnection
